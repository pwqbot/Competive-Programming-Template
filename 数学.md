...
bool miller_rabin(ll p)
{
    ...
    ll a_test[] = {2, 3, 5, 7, 11, 29, 37, 325, 9375, 28178, 450775, 9780504, 1795265022};

    for (ll a : a_test)
    {
        if (a >= p)
            break;
        ll ap = qpow(a, u, p); // (a^u) % p
        ll nap = 0;
        rep(i, 1, t)
        {
            nap = qmul(ap, ap, p); // (a^u, (a^u)^2, ((a^u)^2)^2)
            if (nap == 1 && ap != 1 && ap != p - 1)
                return false;
            ap = nap;
        }
        if (ap != 1)
...
ll pollard_rho(ll n)
{
    ...
    auto f = [&c, n](const ll &x)
    { return ((ll128)qmul(x, x, n) + c) % n; };
    while (1)
    {
        c = rnd() % (n - 1) + 1;
        ll r = 0, t = 0;
        ...
        do
        {
            rep(i, 1, step)
            {
                ...
                if (t == r || (q = (ll128)p * abs(t - r) % n) == 0) // 发现环或者积即将为0, 退出
                    break;
                p = q;
            }
            ll d = gcd(p, n);
...
ll max_prime_factor(ll x) 
{
    ...
    if (d == x)
        return x;
    return max(max_prime_factor(d), max_prime_factor(x / d));
}

// 找所有的质因子
void find_prime_factor(ll x, vector<ll> &v)
{
    ll d = pollard_rho(x);
    if (d == x)
    {
